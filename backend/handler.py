...
def _progress_task(payload, signer, timestamp, state):
    ...
    for task in task_container.entries:
        if task.task_name == payload.task_name:
            ...
            if task.progress == 3:
                # if progress is DONE, reject transaction
                raise InvalidTransaction(
                    "task progress is already DONE")
            if task.progress < 3:
                # set the progress and put it in state
...
def _add_user(payload, signer, timestamp, state):
    ...
    if not project_container: # if no container exists, raise error
        raise InvalidTransaction(
            "project with specified name does not exist")
    # find the project with the correct name
    for project_node in project_container.entries:
        if project_node.project_name == payload.project_name:
            # set the new public keys and put it in state
            project_node.public_keys.append(payload.public_keys)
            _set_container(state, project_address, project_container)
            return
    # if project with specified name not found, reject transaction
    raise InvalidTransaction(
        "project with specified name does not exist")
